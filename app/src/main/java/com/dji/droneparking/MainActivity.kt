...
//import com.dji.droneparking.mission
...
class MainActivity : AppCompatActivity(), GoogleMap.OnMapClickListener, OnMapReadyCallback,
    ...
    private fun cameraUpdate() {
        ...
        val zoomLevel = 18f
        val cameraUpdate = CameraUpdateFactory.newLatLngZoom(LatLng(droneLocationLat, droneLocationLng), zoomLevel)
    ...
    private fun startWaypointMission() {
        getWaypointMissionOperator()?.startMission { error ->
            Toast.makeText(this, "Mission Start: " + if (error == null) "Successfully" else error.description, Toast.LENGTH_SHORT).show()
        }
    }

    private fun stopWaypointMission() {
        getWaypointMissionOperator()?.stopMission { error ->
            Toast.makeText(this, "Mission Stop: " + if (error == null) "Successfully" else error.description, Toast.LENGTH_SHORT).show()
        }
    }
    ...
    private fun uploadWaypointMission() {
        getWaypointMissionOperator()?.uploadMission { error ->
            if (error == null) {
                Toast.makeText(this, "Mission upload successfully!", Toast.LENGTH_SHORT).show()
            } else {
                Toast.makeText(this, "Mission upload failed, error: " + error.description + " retrying...", Toast.LENGTH_SHORT).show()
        ...
        waypointMissionBuilder?.let { builder ->
            ...
            getWaypointMissionOperator()?.let { operator ->
                ...
                if (error == null) {
                    ...
                    Toast.makeText(this, "loadWaypoint failed " + error.description, Toast.LENGTH_SHORT).show()
                }
            }
        }
    }
